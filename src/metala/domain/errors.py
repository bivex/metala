...
class MetalaError(Exception):
    """Base type for all system errors."""


class BusinessRuleViolation(MetalaError):
...
class InputValidationError(MetalaError):
    """Raised for invalid user input at the system boundary."""


class SourceAccessError(MetalaError):
    """Raised when the system cannot access or decode a source file."""


class GeneratedParserNotAvailableError(MetalaError):
