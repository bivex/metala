...
from metala.domain.errors import InputValidationError, SourceAccessError
from metala.domain.model import SourceUnit, SourceUnitId
from metala.domain.ports import SourceRepository
...
class FileSystemSourceRepository(SourceRepository):
    def load_file(self, path: str) -> SourceUnit:
        ...
        if source_path.suffix != ".metal":
            raise InputValidationError(f"expected a .metal file, got: {source_path}")

        return self._load_source_unit(source_path)

    def list_metal_sources(self, root_path: str) -> tuple[SourceUnit, ...]:
        ...
        source_paths = tuple(sorted(path for path in root.rglob("*.metal") if path.is_file()))
        if not source_paths:
            raise InputValidationError(f"no .metal files found under: {root}")
