...
from metala.domain.ports import NassiDiagramRenderer
  ...
  <body>
    <div class="viewer">
      <div class="titlebar">
        ...
        <span class="titlebar-text">Metala · NSD Viewer</span>
      </div>
      <div class="toolbar">
...
"""
    ...
    def _render_step(self, step: ControlFlowStep, *, depth: int) -> str:
        ...
        if isinstance(step, WhileFlowStep):
            return self._render_single_body(step.condition, step.body_steps, depth=depth)
        if isinstance(step, ForInFlowStep):
            return self._render_single_body(step.header, step.body_steps, depth=depth)
